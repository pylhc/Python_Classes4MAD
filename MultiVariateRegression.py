...
def testMe():
   ...
   print "2 dimension"
   myData = []
   for x in range(100): # Data definition
      z = random.randint(0, 100)
      myData.append([6 + 5*x + 4*x*x + 3*x*z + 2*z + 1*z*z, x, z])
   myEquations = []
   myEquations.append(lambda rawItem, coefIndex: 1)    # termino independiente
   myEquations.append(lambda rawItem, coefIndex: rawItem[1])  # x
   myEquations.append(lambda rawItem, coefIndex: rawItem[1] * rawItem[1]) # x*x
   myEquations.append(lambda rawItem, coefIndex: rawItem[1] * rawItem[2]) # x*z
   myEquations.append(lambda rawItem, coefIndex: rawItem[2])              # z
   myEquations.append(lambda rawItem, coefIndex: rawItem[2] * rawItem[2]) # z*z
   print regression(myData, myEquations)

   # multivariate equation: xf = a*pi + b*xi
   xi=[1,2,3,4]
   pi=[1,4,8,16]    
   xf=[3,8,14,24]
   myData = []
   for i in range(len(xi)):
       myData.append([xf[i],pi[i],xi[i]])
   myEquations = []
   myEquations.append(lambda rawItem, coefIndex: rawItem[1] ) #pi
   myEquations.append(lambda rawItem, coefIndex: rawItem[2] ) #xi
   print regression(myData, myEquations)
...
def solveRSquared(rawData, coefficients, xEquationForm, \
   ...
   res=sumsquare / (ysquare - (samples * yaverage * yaverage))
   print "R2=",res
   return res
