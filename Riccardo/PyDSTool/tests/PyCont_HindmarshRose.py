...
from PyDSTool.PyCont.misc import getMaps
...
def full_system():
    ...
    t0 = 1310
    T0 = 452.84
    T = HRtraj.findApproxPeriod(t0, t0+T0, v='z', rtol=0.015)
...
def PRC(HR):
    HR['LC1'].forward()
    pt = HR['LC1'].getSpecialPoint('RG1')
    (J, maps, evec1) = getMaps(pt, 'RG', output=False, screen=False, cyclic=False)
    cycle = pt.labels['RG']['cycle']
    nint = HR['LC1'].NumIntervals
    ncol = HR['LC1'].NumCollocation
    t = cycle['t'][ncol*arange(nint)]
    pylab.figure()
    pylab.plot(t, [vec[0] for vec in evec1], 'r')
    HR['LC1'].plot_cycles(cycles='RG1')
