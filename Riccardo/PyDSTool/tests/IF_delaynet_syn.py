...
def makeIFneuron(name, par_args, IFmodelC):
    ...
    IFneuron = IF(name, compatibleNodes=[IFnetwork], componentTypes=[synchannel],
...
def connectIFneurons(source, target, g, vrev, IFmodelC):
    ...
    syn=synchannel('syn_'+source.name+"_"+target.name, compatibleNodes=[IF],
                  compatibleGens=compatGens)
    syn.add([s,I,g,vrev])
    target.add(syn)
...
for targ in targets:
  ...
  estruct.events[targ+'_sparr'].pushToQ(t+pdict['delay'])
  xdict['"""+source.name+"""_excited'] = 1
  pdict[targ+'_nextspikearr'] = estruct.events[targ+'_sparr'].queue[0]""")

    sparr_epmapping=EvMapping(defString="""xdict['"""+target.name+"_"+syn.name+"""_s']=1
try:
  estruct.events['"""+target.name+"""_sparr'].popFromQ()
  pdict['"""+target.name+"""_nextspikearr'] = estruct.events['"""+target.name+"""_sparr'].queue[0]
except IndexError:
...
IFmspec = IFnetwork('IFnet_delay', componentTypes=[IF],
                    compatibleGens=compatGens)
IFmspec.add([delay, vreset, threshval, b_syn, vspike, tspike])

genAlgPars={'rtol': 1e-2, 'atol': 1e-3, 'max_step': 0.5,
                                'init_step': 0.1}
if targetGen == 'Vode_ODEsystem':
    # Vode fails if don't use stiff option
    genAlgPars['stiff']=True

IFmodelC.addGenInfo(IFmspec, targetGen,
...
connectIFneurons(IF1, IF2, 1.0, -80, IFmodelC)
connectIFneurons(IF2, IF1, 1.0, -80, IFmodelC)
IFmspec.add([IF1, IF2])
...
icdict = {'IF1.V': -85, 'IF2.V': -65, 'IF1.excited': 0, 'IF2.excited': 0,
