...
def makeIFneuron(name, par_args):
    v=Var("(I - gl*(V-vl))/C", 'V', domain=[-120,-50], specType='RHSfuncSpec')
    ...
    sparr_ev = Events.makeZeroCrossEvent("globalindepvar(t)-"+name+"_nextspikearr",
                                         1, sparr_ev_args,
                                         parnames=[name+"_nextspikearr"],
                                         targetlang=targetlang)
    nextT = Par('0', "nextspikearr")

    IFneuron = IF(name, compatibleNodes=[IFnetwork],
...
IFmspec = IFnetwork('IFnet', componentTypes=[IF],
                    compatibleGens=['Vode_ODEsystem', 'Dopri_ODEsystem'])
IFmspec.add([delay, vreset, threshval, syn_sign, syn_strength])

genAlgPars = {'rtol': 1e-3, 'atol': 1e-4, 'init_step': 0.01}
if targetGen == 'Vode_ODEsystem':
