...
_classes = ['Function', 'TestFunc', 'AddTestFunction', 'DiscreteMap', 'FixedPointMap',
            ...
            'Fold_Det', 'Fold_Bor', 'Branch_Det', 'Branch_Bor', 'Hopf_Det', 'Hopf_Bor',
...
class AddTestFunction(Function):
    ...
    def func(self, X):
        self.sysfunc.J_coords = self.sysfunc.jac(X, self.sysfunc.coords)
        self.sysfunc.J_params = self.sysfunc.jac(X, self.sysfunc.params)
        return c_[self.sysfunc(X), self.testfunc(X, None)]
    ...
    def func(self, X):
        ...
        if self.period >= 2:
            for k in range(2,self.period+1):
                FX = self.F(c_[FX, X[self.F.params]])
        return FX
...
class B_Check(TestFunc):
    ...
    def __init__(self, F, C, save=False, numpoints=None):
        ...
        for par in C.freepars:
            domain = C.gensys._pdomain[par]
            lower[par] = domain[0]
            upper[par] = domain[1]
        
        for par in C.gensys.funcspec.vars:
            domain = C.gensys._xdomain[par]
            lower[par] = domain[0]
            upper[par] = domain[1]
