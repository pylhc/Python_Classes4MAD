...
#
# Version 1.3.5
#   Added support for variable and parameter bounds to appear in events.
#   Added support for external inputs in events.
#   Adapted event class __setitem__ to accept lists of events.
#
# Version 1.3.4
#   Added Evtimes to eventstruct to permit event interval
#     checks for high-level events.
#
# Version 1.3.3
#   Now store plain text definition of event as attribute '_expr'.
#   Modified comments and function names to reflect fact that there are now
#     more than one external ("low-level") target languages.
#   Added Matlab event support for ADMC++.
#   Specification of events can now be done with ModelSpec Quantities.
#   Added arguments to C specification event functions to be compatible
#     with external inputs.
#
# Version 1.3.2
#   Updated processReusedXX functions to reflect change in FuncSpec.py
#
# Version 1.3.1
#   Converted all variables .values() to .itervalues()
#   Added event queue and temporary parameter / storage attributes.
#   Added more exception information for those raised in _fn.
#
# Version 1.3
#   Removed DomainTest.
#   Unified making High- and Low-level events, so that low level events
#     have a high-level counterpart for use in domain inclusion testing by
#     Model instances (to determine initial generator) and for post-processing
#     of trajectory data for events (C-code event functions are not exposed
#     to the user).
#   Renamed makeHL_UserEvent to makeZeroCrossEvent (which now incorporates
#     both low and high level events).
#
# Version 1.2.5
#   setTermFlag() and setActiveFlag() now accept lists of event names.
#
# Version 1.2.4
#   Added support for events with local term definitions (including "reused"
#     terms).
#   Added __call__ to classes.
#   Provided functionality from call to Event.
#   Added eventinterval field to Event.
#
# Version 1.2.3
#   Fixed low level events to permit non-precise events.
#
# Version 1.2.2
#   Added contains method to HighLevelEvent and DomainTest.
#   Fixed bug in searchForEvents when eventdelay prevents early event from
#    being detected -- allowed eventdelay to be temporarily switched off.
#   High-level user event no longer needs exprname as first argument, making
#    it more compatible with the low-level version.
#
# Version 1.2.1
#   Fixed sign error bug in searchForEvents.
#   Fixed bug with eventdelay in __call__.
#
# Version 1.2
#   Added support for events implemented in C code.
# ----------------------------------------------------------------------------
...
class EventStruct(object):
    ...
    def __init__(self):
        ...
        self._keylist = ['term', 'nonterm', 'active', 'varlinked', 'precise',
                         'notvarlinked', 'notactive', 'highlevel', 'lowlevel']
        self._filterDict = {}
    ...
    def query(self, keylist, eventlist=None):
        """keylist, eventlist=None -> eventlist
    ...
    def resetHighLevelEvents(self, t0, eventlist=None, state=None):
        ...
        if type(state)==list:
            if len(state) != len(eventlist):
                raise ValueError, \
                      "Mismatch between length of state list argument and eventlist"
    ...
    # this method is somewhat unused -- may be taken out
...
##        self.errors = []
        ...
        if 'noHighLevel' in kw:
            ...
            self.addMethods()
        if 'prevsign' in kw:
            # set initial value -- useful for one-off tests
            self.prevsign = kw['prevsign']
        else:
            self.prevsign = None
        ...
        # event queue information (if used for discrete delays)
        self.queue = []
    ...
    # queues are intended to be of Floats, for times at which next terminal
    # event should occur. This feature is in development.
    def pushToQ(self, item):
        self.queue.append(item)

    def addToQ(self, item):
        self.queue.append(item)
        self.queue.sort()
    ...
    def __call__(self, t=None, varDict=None, parDict=None):
        ...
        if varDict is None:
            ...
            if self.prevsign is None:
                try:
                    self.prevsign = scipy.sign(self._fn(t, parDict))
                except:
                ...
                try:
                    fval = scipy.sign(self._fn(t, parDict))
                except:
                    print "Error in event", self.name
                    info(parDict, "Parameters")
                    raise
                if self.dircode == 0:
                    result = self.prevsign != fval
                else:
                    result = self.prevsign != fval and \
            ...
            if self.prevsign is None:
                try:
                    self.prevsign = scipy.sign(self._fn(varDict_temp, parDict))
                except:
                    print "Error in event", self.name
                    info(parDict, "Parameters")
                    print "\n"
                ...
                try:
                    fval = scipy.sign(self._fn(varDict_temp, parDict))
                except:
                    print "Error in event", self.name
                    info(parDict, "Parameters")
                    print "\n"
    ...
    def searchForEvents(self, trange=None, dt=None, checklevel=2,
        ...
        if discretevars:
            ...
            if dt > trange[1]-trange[0]:
                ...
                raise ValueError, 'dt (eventtol if not specified) is too large for trange'
...
class LowLevelEvent(Event):
    ...
    def __init__(self, kw):
        ...
        dummyQ.mapNames({'abs': 'fabs', 'sign': 'sgn'})
        if dummyQ[0] != 'return':
        ...
        for symb in expQS.freeSymbols:
            if symb in allvars:
                ...
                if spectype == 'RHSfuncSpec':
                    ...
                    # cannot depend on eachother, in a well-defined system.
...
##        raise NotImplementedError, "Inputs to non C-based events are not yet supported."
    ...
    if varnames == [] and inputnames == [] and 't' not in expQS:
        raise ValueError("Time must appear in expression")
...
# (Adapted from scipy.optimize.minpack.bisection code)
