  ...
  def __init__(self, filename, regexp=None):
    ...
    for line in open(filename):
      ...
      if (f[0] == '@'):  # descriptor lines
        if ('s' in f[2]):
          f[3]=line[line.index('"'):-1]
        self.descs[f[1]]=self.conv(f[2],f[3])
      elif ( f[0] == '*'): # labels lines
        f.pop(0) ; labels=f
        for l in labels: self.data[l]=[]
      elif (f[0] == '$'):  # type lines
        f.pop(0) ; types=f
      else :   # data lines
        if (regexp):
...
#      setattr(self,l,self.descs[l])
  ...
  def pattern(self,regexp) :
    """
    create a list based on the selection
    t.select(t.pattern("IP"),t.name,t.s)
    """
  ...
  def elem(self,ra) :
    """
    return a list of index for name=ra
  ...
  def select(self,index,*a) :
    """
    create a list based on the a list of index"
    t.select(t.pattern("MQ"),t.name,t.s)
    t.select(t.range("IP","IR11"),t.name,t.s)
    """
#    print type(a)
    if (len(a) == 1) :
      d=list(a[0].take(index))
    else :
      d=[ list(i.take(index)) for i in a ]
      d=zip(*d)
      d=map(list,d)
    return d
