...
def sigmacore(mapxory_i_nn_mm):
        mapxory,i,nn,mm=mapxory_i_nn_mm
        sx2=0.
        for coeff1 in mapxory[nn:mm]:
...
#########################
    ...
    def sigmawrap(self, xory, i , nproc):
        '''
        ...
        for j in range(nproc):
            nn=j*period
            mm=(j+1)*period
            if mm >(nterms-1): mm=nterms-1
            input.append([mapxory, i, nn,mm])
        results=pool.map(sigmacore, input)
        return sum(results)
...
if __name__ == '__main__':
    ...
    a=Map(order=18)
    print a.sigmawrap('x',  sigmaFFSstart, 1)
