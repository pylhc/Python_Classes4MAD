...
#########################
    g = open ('od_parameters.tcl', 'w')
    g.write('set kod1 '+str(deltafamilie[0])+'\n')
    g.write('set ratio '+str(deltafamilie[1])+'\n')
    g.write('set odsep '+str(deltafamilie[2])+'\n')
    g.write('set oddrift '+str(deltafamilie[3])+'\n')        
    g.close()
    return
...
#########################
    global minx, maxx, miny, maxy, weis
    print deltafamilies
    writeparams(deltafamilies)
    system('placet trackbds.tcl > scum')
    system('./MaxMin particles.OD4o.out')
    readdist('dists')
    print minx, maxx, miny, maxy
    return ((maxx-minx)*weis[0]+ (maxy-miny)*weis[1])*deltafamilies[0]*deltafamilies[0]*deltafamilies[1]*deltafamilies[1]
...
##############
    ...
    weis=[1,0.01]
    deltafamilies1=[-50000,  0.5,  0,  0]
    incr=[1000,0.1,0.1,0.1]
