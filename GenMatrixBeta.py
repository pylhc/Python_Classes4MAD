# Generate  Matrix for beta correction, under editing.
...
##########################################
	...
	################################
	def computevectorEXP(self,x,y,bx,by,dx):
	###############################################################################
		phix=[]
		phiy=[]
		betx=[]
		...
		for b in self.betaxlist:
			bx.append(bx.BETX[dx.indx[b]])
		for b in self.betaylist:
			by.append(by.BETY[dy.indx[b]])
	...
	################################################################################
    		...
    		self.normvector=array(concatenate([ones(len(self.phasexlist)), ones(len(self.phaseylist)), ones(len(self.betxlist)), ones(len(self.betylist)), ones(ndisp+2)]))*1.0  #To take dbeta/beta later
